...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
  ...
  local/data_prep.py --dataset_dir ${dataset_dir}
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
  ...
  dump.sh --cmd "$train_cmd" --nj 10 --do_delta ${do_delta} \
      data/${test_set}/feats.scp data/${train_set}/cmvn.ark exp/dump_feats/${test_set} ${feat_recog_dir}
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    for rtask in ${train_dev} ${test_set}; do
    (
